...
class MapSedanDataAdapterTest extends Specification {
    ...
    def "getObjectValue(): returns raw Integer without toString conversion"() {
        given:
        def adapter = new MapSedanDataAdapter([count: 42])

        expect:
        adapter.getObjectValue('count') == 42
        adapter.getObjectValue('count') instanceof Integer
    }

    def "getObjectValue(): returns null for missing key"() {
        expect:
        new MapSedanDataAdapter([a: 1]).getObjectValue('missing') == null
    }

    def "getObjectValue(): returns raw String value"() {
        expect:
        new MapSedanDataAdapter([name: 'Alice']).getObjectValue('name') == 'Alice'
    }
}